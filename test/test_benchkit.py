...
def test_sweep_runs_cases_and_produces_frame(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    build_perf.sweep(cases=[{"size": 8}, {"size": 16}], show_progress=False)
    frame = bk.load_frame("build-perf")

    assert len(frame) == 2
    assert set(frame.columns) == {"config.size", "result.compile_time_ms", "result.throughput"}
    assert sorted(frame["config.size"]) == [8, 16]


def test_load_frame_metadata(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("BENCHKIT_HOME", str(tmp_path / "home"))

    @bk.func("meta-test")
    def meta_test(n: int) -> None:
        bk.context().save_result({"v": float(n)})

    meta_test.sweep(cases=[{"n": 1}], show_progress=False)
    frame = bk.load_frame("meta-test", metadata=True)

    assert "status" in frame.columns
    assert "rep" in frame.columns
    assert "case_key" in frame.columns
    assert "created_at" in frame.columns
...
def test_single_call_returns_run(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert bk.get_run("single-case", config={"size": 8}, status=bk.RunStatus.OK).metrics == {"compile_time_ms": 8.0}
...
def test_multiple_save_result_creates_repetitions(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    repeated.sweep(cases=[{"size": 8}], show_progress=False)
    runs = bk.load_runs("repetitions", status=bk.RunStatus.OK)

    assert len(runs) == 3
    assert [r.rep for r in runs] == [0, 1, 2]
    assert [r.metrics["time_ms"] for r in runs] == [8.0, 9.0, 10.0]

    frame = bk.load_frame("repetitions", metadata=True)
    assert len(frame) == 3
...
def test_load_runs_reopens_stored_results(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("BENCHKIT_HOME", str(tmp_path / "home"))
    ...
    frame = bk.load_frame("reopen-study")
    run = bk.get_run("reopen-study", config={"size": 8}, status=bk.RunStatus.OK)
...
def test_explicit_case_objects_are_supported(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    case_object_benchmark.sweep(
        cases=[Case(size=8, backend="cpu"), Case(size=16, backend="gpu")],
        show_progress=False,
    )

    assert bk.get_run(
        "case-object-benchmark", config={"size": 8, "backend": "cpu"}, status=bk.RunStatus.OK
    ).metrics == {"compile_time_ms": 8.0}
    assert bk.get_run(
        "case-object-benchmark", config={"size": 16, "backend": "gpu"}, status=bk.RunStatus.OK
    ).metrics == {"compile_time_ms": 16.0}
...
def test_run_folders_are_nested_by_benchmark_and_sweep(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    nested_layout.sweep(cases=[{"size": 8}], show_progress=False)
    run = bk.get_run("nested-layout", config={"size": 8}, status=bk.RunStatus.OK)
...
def test_latest_sweep_used_by_default(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("BENCHKIT_HOME", str(tmp_path / "home"))
    ...
    # First sweep
    s1 = default_sweep_benchmark.sweep(cases=[{"size": 8}], show_progress=False)

    # Second sweep (fresh by default)
    s2 = default_sweep_benchmark.sweep(cases=[{"size": 16}], show_progress=False)

    assert s1 != s2
    assert bk.get_run("default-sweep-benchmark", config={"size": 16}, status=bk.RunStatus.OK).metrics == {
...
def test_failed_runs_expose_error_metadata(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    failing_benchmark.sweep(cases=[{"size": 8}], show_progress=False)
    run = bk.get_run("failing-benchmark", config={"size": 8}, status=bk.RunStatus.FAILURE)
...
def test_parallel_sweep_produces_correct_results(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    parallel_benchmark.sweep(
        cases=[{"size": s} for s in range(1, 9)],
        show_progress=False,
        max_workers=4,
    )

    runs = bk.load_runs("parallel-benchmark", status=bk.RunStatus.OK)
    assert len(runs) == 8
...
def test_default_sweep_starts_fresh(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    s1 = fresh_sweep.sweep(cases=[{"size": 1}], show_progress=False)

    # By default, second call starts a fresh sweep (re-runs cases)
    s2 = fresh_sweep.sweep(cases=[{"size": 1}], show_progress=False)
    assert s2 != s1
    assert calls["count"] == 2

    # With resume=True, resumes the latest sweep (skips completed)
    s3 = fresh_sweep.sweep(cases=[{"size": 1}], show_progress=False, resume=True)
    assert s3 == s2
    assert calls["count"] == 2

    # BENCHKIT_RESUME=1 env var also triggers resume
    monkeypatch.setenv("BENCHKIT_RESUME", "1")
    s4 = fresh_sweep.sweep(cases=[{"size": 1}], show_progress=False)
    assert s4 == s2
    assert calls["count"] == 2
